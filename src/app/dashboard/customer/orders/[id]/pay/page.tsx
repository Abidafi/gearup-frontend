...
import { Calendar, Tag, ArrowLeft, Loader2, ShieldCheck, Package } from 'lucide-react';
...
export default function GearDetailsPage() {
  ...
  const handleRentNow = async () => {
    ...
    const calculatedTotalPrice = diffDays * pricePerDay * quantity;
          ...
          <div className="p-8 flex flex-col justify-between space-y-6">
            ...
            <div className="border-t border-slate-900 pt-6 space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="space-y-1">
                  ...
                  <input 
                    ...
                    className="w-full bg-slate-800 border border-slate-700 rounded-xl p-3 text-white text-sm focus:outline-none focus:border-blue-600 transition-colors" 
                  />
                </div>
                <div className="space-y-1">
                  <label className="text-xs font-medium text-slate-400 flex items-center gap-1.5">
              ...
              {availableStock > 0 && (
                <div className="space-y-1">
                  <label className="text-xs font-medium text-slate-400 flex items-center gap-1.5">
                    <Package className="h-3.5 w-3.5 text-blue-500" /> Quantity to Rent
                  </label>
                  <select
                    value={quantity}
